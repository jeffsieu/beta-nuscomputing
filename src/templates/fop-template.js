    ...
    return (
      <Box mt={2} style={{ display: 'flex', flexDirection: 'column' }} justifyContent='center' alignItems='center' >
        <Avatar className={classes.large} alt={person.name}>
          <GatsbyImage image={getImage(images[person.name.replace(/ |,/g, '')])} />
        </Avatar>
        <Box mt={1}>
...
`);
  ...
  console.log(gallery);
  return (
    <BaseContainer title={event.name} background={bannerImage} backgroundPosition={event.banner_position}>
      <Box mb={4}>
                ...
                event.gallery.map((photo, index) => {
                  ...
                  return (
                    <Box mt={4} style={{position: 'relative'}}>
                      ...
                      <GatsbyImage
                        image={getImage(gallery[photo.url])}
